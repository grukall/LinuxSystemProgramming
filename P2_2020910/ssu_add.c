...
int main(int argc, char *argv[])
{
    ...
    if (argv[1] == NULL)
    {
        fprintf(stderr, "ERROR: <PATH> is not include\nUsage: add <PATH> : record path to staging area, path will not tracking modification\n");
        exit (1);
    }
    ...
    //현재 디렉토리를 벗어난 경로나, .repo를 포함한 경로를 입력하면 에러
    ...
    if (check_already_backuped(path)) {
        if (skip_ptr == NULL)
            fprintf(stderr, "\".\" already exist in staging area\n");
        else
            fprintf(stderr, "\"./%s\" already exist in staging area\n", skip_ptr); 
        exit(1);
    }

    // 위 과정을 모두 통과하면 stagging.log에 추가
...
int check_already_backuped(char* path) {
    ...
    if (curr_dir->next_dir->subdir_head != NULL)
    {
        ...
        {
            if (temp_dir->next_dir->stag && strcmp(substr(temp_dir->next_dir->dir_path, 0 , strlen(temp_dir->next_dir->dir_path) - 1), path) == 0)
                return 1;
